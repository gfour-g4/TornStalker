...
// ═══════════════════════════════════════════════════════════════
// NOTIFICATIONS - Icons & Activities
// ═══════════════════════════════════════════════════════════════

function iconStarted(type, icon) {
  const config = TRACKABLE_ICONS[type];
  const action = ICON_ACTIONS[type] || {};
  
  const embed = new EmbedBuilder()
    .setColor(COLORS.info)
    .setTitle(`${config.emoji} ${config.name} Started!`)
    .setTimestamp();
  
  const lines = [];
  
  if (icon.until) {
    lines.push(`**Ends:** ${discordTimestamp(icon.until, 'R')} (${discordTimestamp(icon.until, 't')})`);
  }
  
  if (icon.description) {
    lines.push(`> ${icon.description.replace(/<[^>]*>/g, '')}`);
  }
  
  if (action.link) {
    lines.push(`🔗 [Go Now →](${action.link})`);
  }
  
  embed.setDescription(lines.join('\n') || action.text || 'Activity started!');
  
  return embed;
}

function iconEnded(type, icon) {
  const config = TRACKABLE_ICONS[type];
  
  return new EmbedBuilder()
    .setColor(COLORS.good)
    .setTitle(`${config.emoji} ${config.name} Ended`)
    .setDescription(icon.description?.replace(/<[^>]*>/g, '') || 'Activity completed!')
    .setTimestamp();
}

function racingFinished(icon) {
  // Parse result from description if available
  // Example: "You finished <b>2nd</b> in the Meltdown race..."
  const desc = icon.description?.replace(/<[^>]*>/g, '') || 'Race completed!';
  
  return new EmbedBuilder()
    .setColor(COLORS.good)
    .setTitle('🏎️ Race Finished!')
    .setDescription(desc)
    .addFields({
      name: '🔗 Racing',
      value: `[View Results](${LINKS.racing})`,
      inline: true,
    })
    .setTimestamp();
}
...
// ═══════════════════════════════════════════════════════════════
// NOTIFICATIONS - Company Addiction
// ═══════════════════════════════════════════════════════════════

function addictionAlert(current, threshold, employeeData) {
  const embed = new EmbedBuilder()
    .setColor(COLORS.bad)
    .setTitle(`${EMOJI.addiction} Addiction Too High!`)
    .setDescription([
      `Your addiction penalty is **${current}%** (threshold: ${threshold}%)`,
      '',
      '⚠️ This is reducing your company effectiveness!',
    ].join('\n'))
    .setTimestamp();
  
  if (employeeData?.effectiveness) {
    const eff = employeeData.effectiveness;
    embed.addFields({
      name: '📊 Effectiveness Breakdown',
      value: [
        `Working Stats: **+${eff.working_stats || 0}**`,
        `Settled In: **+${eff.settled_in || 0}**`,
        `Merits: **+${eff.merits || 0}**`,
        eff.director_education ? `Education: **+${eff.director_education}**` : null,
        `Addiction: **${eff.addiction || 0}**`,
        `**Total: ${eff.total || 0}**`,
      ].filter(Boolean).join('\n'),
      inline: false,
    });
  ...
  embed.addFields({
    name: '💡 Solution',
    value: `Wait for addiction to wear off, or use [Medical Items](${LINKS.medical})`,
    inline: false,
  });
...
function addictionRecovered(current, threshold) {
  return new EmbedBuilder()
    .setColor(COLORS.good)
    .setTitle(`${EMOJI.success} Addiction Recovered!`)
    .setDescription([
      `Your addiction penalty is now **${current}%**`,
      `(above threshold of ${threshold}%)`,
      '',
      '✅ Back to normal effectiveness!',
    ].join('\n'))
    .setTimestamp();
}
